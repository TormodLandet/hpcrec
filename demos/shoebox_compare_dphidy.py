...
def shoebox_demo(N, L, h=1, k=1, show_plot=True, neumann=False, refine_y=0.0, fem=0):
    ...
    for i in range(Nd):
        ...
        domain.dof_coordinates[i] = (beta_x * xc + (1 - beta_x) * x, beta_y * yc + (1 - beta_y) * y)
    ...
    if dy1 == dy2:
        ...
        d_phi_dy_fdm3 = (
            11 * phi_layer_1 - 18 * phi_layer_2 + 9 * phi_layer_3 - 2 * phi_layer_4
        ) / (6 * dy)
    else:
        # Sympy derived expresions for dy.. != dy..
        d_phi_dy_fdm1 = (phi_layer_1 - phi_layer_2) / dy1
        d_phi_dy_fdm2 = (
            2 * phi_layer_1 * dy1 * dy2
            + phi_layer_1 * dy2**2
            - phi_layer_2 * dy1**2
            - 2 * phi_layer_2 * dy1 * dy2
            - phi_layer_2 * dy2**2
            + phi_layer_3 * dy1**2
        ) / (dy1 * dy2 * (dy1 + dy2))
        d_phi_dy_fdm3 = (
            3 * phi_layer_1 * dy1**2 * dy2**2 * dy3
            + 3 * phi_layer_1 * dy1**2 * dy2 * dy3**2
            + 4 * phi_layer_1 * dy1 * dy2**3 * dy3
            + 6 * phi_layer_1 * dy1 * dy2**2 * dy3**2
            + 2 * phi_layer_1 * dy1 * dy2 * dy3**3
            + phi_layer_1 * dy2**4 * dy3
            + 2 * phi_layer_1 * dy2**3 * dy3**2
            + phi_layer_1 * dy2**2 * dy3**3
            - phi_layer_2 * dy1**4 * dy3
            - 4 * phi_layer_2 * dy1**3 * dy2 * dy3
            - 2 * phi_layer_2 * dy1**3 * dy3**2
            - 6 * phi_layer_2 * dy1**2 * dy2**2 * dy3
            - 6 * phi_layer_2 * dy1**2 * dy2 * dy3**2
            - phi_layer_2 * dy1**2 * dy3**3
            - 4 * phi_layer_2 * dy1 * dy2**3 * dy3
            - 6 * phi_layer_2 * dy1 * dy2**2 * dy3**2
            - 2 * phi_layer_2 * dy1 * dy2 * dy3**3
            - phi_layer_2 * dy2**4 * dy3
            - 2 * phi_layer_2 * dy2**3 * dy3**2
            - phi_layer_2 * dy2**2 * dy3**3
            + phi_layer_3 * dy1**4 * dy2
            + phi_layer_3 * dy1**4 * dy3
            + 2 * phi_layer_3 * dy1**3 * dy2**2
            + 4 * phi_layer_3 * dy1**3 * dy2 * dy3
            + 2 * phi_layer_3 * dy1**3 * dy3**2
            + phi_layer_3 * dy1**2 * dy2**3
            + 3 * phi_layer_3 * dy1**2 * dy2**2 * dy3
            + 3 * phi_layer_3 * dy1**2 * dy2 * dy3**2
            + phi_layer_3 * dy1**2 * dy3**3
            - phi_layer_4 * dy1**4 * dy2
            - 2 * phi_layer_4 * dy1**3 * dy2**2
            - phi_layer_4 * dy1**2 * dy2**3
        ) / (
            dy1
            * dy2
            * dy3
    ...
    # Print the errors
    print(f"Error phi          : {numpy.linalg.norm(phi_h - phi_a):15.8e}")
    print(f"Error d_phi_dy     : {numpy.linalg.norm(d_phi_dy_h - d_phi_dy_a):15.8e}")
    print(f"Error d_phi_dy FDM1: {numpy.linalg.norm(d_phi_dy_fdm1 - d_phi_dy_a):15.8e}")
    print(f"Error d_phi_dy FDM2: {numpy.linalg.norm(d_phi_dy_fdm2 - d_phi_dy_a):15.8e}")
    print(f"Error d_phi_dy FDM3: {numpy.linalg.norm(d_phi_dy_fdm3 - d_phi_dy_a):15.8e}")
...
def get_dofs(coords, vert_pos, eps=1e-6):
    ...
    idx = numpy.where((coords[:, 1] > vert_pos - eps) & (coords[:, 1] < vert_pos + eps))[0]

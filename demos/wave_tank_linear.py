        ...
        if it % 30:
            ...
            for ie in range(Nf):
                if ie == 0:
                    ...
                    eta_smoothed[ie] = (
                        eta[it, ie - 1] + eta[it, ie] + eta[it, ie + 1] + eta[it, ie + 2]
                    ) / 4
                elif ie == Nf - 2:
                    eta_smoothed[ie] = (
                        eta[it, ie - 2] + eta[it, ie - 1] + eta[it, ie] + eta[it, ie + 1]
                    ) / 4
                elif ie == Nf - 1:
...
def save_results(
    file_name: str | Path,
    wave_tank_input: WaveTankInput,
    xpos: np.ndarray,
    tvec: np.ndarray,
    eta: np.ndarray,
):
    data = {"wave_tank_input": wave_tank_input, "xpos": xpos, "tvec": tvec, "eta": eta}
...
if __name__ == "__main__":
    ...
    parser.add_argument("-Nt", type=int, default=101, help="Number of time steps")
